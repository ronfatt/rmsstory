      ...
      <div className="page-shell">
          ...
          <div className="glass rounded-[34px] p-7 md:p-8">
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              {hasChapters ? (
                ...
                <div className="min-w-[240px] rounded-full bg-slate-300 px-7 py-4 text-center text-sm font-semibold text-slate-600">
                  ⏳ Bab pertama akan datang
                </div>
              )}
              <button
                type="button"
            ...
            <div className="mt-7 grid gap-3 rounded-[24px] border border-[var(--border)] bg-white/75 p-5 text-sm font-semibold text-[var(--foreground)] md:grid-cols-3">
              <p>
                📖 {hasChapters ? `Bab ${latestChapter.number} daripada ${totalPlannedChapters}` : "Bab pertama sedang disiapkan"}
              </p>
              <p>🟢 {novel.status}</p>
        ...
        <section className="mt-10 glass rounded-[34px] p-7 md:p-8">
          ...
          {hasChapters ? (
            ...
            <div className="mt-6 rounded-[24px] border border-[var(--border)] bg-white/75 p-5 text-sm leading-7 text-[var(--muted)]">
              Bab belum dibuka lagi. Bila bab pertama diterbitkan, reaksi pembaca akan muncul di sini.
            </div>
          )}
        </section>
        ...
        <section className="mt-10 glass rounded-[34px] p-7 md:p-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-semibold text-[var(--accent-deep)]">
                {hasChapters ? "📚 Mula dari Bab 1" : "📚 Bab akan datang"}
              </h2>
            </div>
            {hasChapters ? (
          ...
          {hasChapters ? (
            ...
            <div className="mt-8 rounded-[26px] border border-[var(--border)] bg-white/76 p-6 text-sm leading-7 text-[var(--muted)]">
              Novel ini sudah diterbitkan, tetapi bab pertama masih belum dibuka. Sila kembali semula mengikut jadual kemas kini.
            </div>
          )}
        </section>
      </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--border)] bg-[rgba(255,247,240,0.94)] px-4 py-3 backdrop-blur md:hidden">
        <div className="page-shell">
          {hasChapters ? (
            ...
            <div className="block rounded-full bg-slate-300 px-6 py-4 text-center text-sm font-semibold text-slate-600">
              ⏳ Bab pertama akan datang
            </div>
          )}
        </div>
